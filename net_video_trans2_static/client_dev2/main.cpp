...
*/
	...
	printf("get buf success\n");
        if(buf.bytesused == IMAGE_HIGH*IMAGE_WIDTH*2){
            ...
            printf("data size is%d", buf.bytesused);
        }
        memcpy(image_buf, buffers[buf.index].start, buf.bytesused);
/*        unsigned char Y[IMAGE_HIGH][IMAGE_WIDTH];
        unsigned char U[IMAGE_HIGH][IMAGE_WIDTH];
        unsigned char V[IMAGE_HIGH][IMAGE_WIDTH];
        for(int k = 0; k < IMAGE_HIGH*IMAGE_WIDTH; k++){
            unsigned char *ptr;
            ptr = &Y[0][0];
            *(ptr + k) = (unsigned char)image_buf[2*k];
        }
        for(int k = 0; k < IMAGE_HIGH*IMAGE_WIDTH; k+=2){
            unsigned char *ptr;
            ptr = &U[0][0];
            *(ptr + k) = (unsigned char)image_buf[2*k+1];
            *(ptr + k + 1) = (unsigned char)image_buf[2*k+1];
        }
        for(int k = 0; k < IMAGE_HIGH*IMAGE_WIDTH; k+=2){
            unsigned char *ptr;
            ptr = &V[0][0];
        ...
        imencode(".jpg", frame_output, data_encode);
	char data_send[400000] ={0};
	int m = 0;
	for(int n : data_encode){
		data_send[m] = (char)n;
		m++;
	}
        int image_size;
        image_size = (int)data_encode.size();
//        imshow("output", frame_output);
        ioctl(camerafd, VIDIOC_QBUF, &buf);
	printf("image_size: %d", image_size);
        push_image(sockfd, (char *)&image_size, sizeof(image_size));
        char echo[2] = {0};
        while (echo[0] != '2') {
            read(sockfd, echo, 2);
            usleep(10);
//            printf("###: %s\n", echo);
	...
	printf("size send ok\n");
        ...
        push_image(sockfd, data_send, (size_t)image_size);
	printf("push image finish\n");
	char echo_2[2] = {0};
        while (echo_2[0] != '1') {
            read(sockfd, echo_2, 2);
            usleep(10);
//            printf("###: %s\n", echo_2);
        }
        printf("%d\n", x);
